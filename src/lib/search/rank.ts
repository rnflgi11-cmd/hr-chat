...
export function makeScorer(params: { q: string; used: string[]; anchors: string[] }) {
  ...
  return function scoreRow(r: Row) {
    ...
    if (/(며칠|일수|몇일|몇\s*일)/.test(q) && /(경조|휴가)/.test(q)) {
      ...
      if (/(경조\s*휴가|경조휴가|경조유형|조의|조문|부고|사망|결혼)/.test(hay)) s += 25;
      if (/(유의사항|사전\s*휴가\s*신청\s*사유|사후\s*휴가\s*신청\s*사유)/.test(hay) && !/(\d+\s*일|휴가일수|일수)/.test(hay)) s -= 45;
      if (/(안식년|선연차|프로젝트\s*수당|프로젝트수당|화환\s*신청|화환신청)/.test(hay)) s -= 80;
    }

    
    // 기타휴가 질문은 경조(결혼/조사/출산/사망) 섹션으로 튀는 것을 강하게 방지
    if (/(기타\s*휴가|기타휴가|예비군|민방위|병역의무|직무\s*교육|교육\s*참석|병가)/.test(q)) {
      if (/(기타|병역의무|민방위|예비군|직무\s*교육|교육\s*참석|병가|훈련\s*증명서|연차\s*차감\s*없음|사전\s*기안)/.test(hay)) s += 70;
      if (/(경조|결혼|조사|사망|부고|조의|조문|출산|조위금|경조금)/.test(hay)) s -= 120;
      if (/(경조\s*휴가|경조휴가)/.test(hay) && !/(예비군|민방위|병역의무)/.test(q)) s -= 80;
    }
